...
export default function BlogCard({ post }: BlogCardProps) {
  ...
  return (
    <article
      className={`
    ${isGradient ? `${gradientClass} animate-gradient-slow` : gradientClass}
    group cursor-pointer lg:col-auto 
    ...
    >
      <a
        href={`/blog/${post.slug}`}
        className={`
          relative flex flex-col justify-start p-4 rounded-2xl
      ...
      >
        ...
        <div className='flex justify-between items-baseline mb-2 md:mb-4 xl:mb-6'>
          <time
            className='text-xs md:text-sm font-mono font-normal uppercase text-gray-300 dark:text-gray-400'
            dateTime={post.date}
          >
